...
// Content возвращает ссылку на слайс документов
func (db *DB) Content() *[]crawler.Document {
	return &db.docs
}

// Doc возвращает ссылку на документ с указанным ID
func (db *DB) Doc(id int) *crawler.Document {
	for i := range db.docs {
		if db.docs[i].ID == id {
			return &db.docs[i]
		}
...
// Del удаляет документ с указанным ID
func (db *DB) Del(id int) bool {
	for i := range db.docs {
		if db.docs[i].ID == id {
			db.docs = append(db.docs[:i], db.docs[i+1:]...)
			return true
		}
	}
	return false
