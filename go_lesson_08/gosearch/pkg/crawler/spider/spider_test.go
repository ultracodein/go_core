...
func TestService_Scan(t *testing.T) {
	...
	// из-за рекурсии в parse порядок элементов в slice может меняться от запуска к запуску:
	// сортируем slice-ы, чтобы порядок не влиял на успешное выполнение теста
